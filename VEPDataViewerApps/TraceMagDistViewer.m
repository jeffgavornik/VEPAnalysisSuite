function varargout = TraceMagDistViewer(varargin)
...
% Set the default CDF plot behavior
dataKeys = {'CDF' 'ecdf_x' 'ecdf_f'};
...
managerObj = TMDAPlotManagerClass(dataKeys{2},dataKeys{3});
...
handles.TMDATemplate = getDataSpecifierTemplate('TMDA');
...
handles.LHXLabel = xlabel(handles.LHaxes,'v (\muV)','fontsize',12,...
    'fontweight','bold');
handles.LHYLabel = ylabel(handles.LHaxes,'CDF','fontsize',12,...
    'fontweight','bold');
handles.RHXLabel = xlabel(handles.RHaxes,'v (\muV)','fontsize',12,...
    'fontweight','bold');
handles.RHYLabel = ylabel(handles.RHaxes,'CDF','fontsize',12,...
    'fontweight','bold');
...
handles.TMDATemplate.resetDataPath();
handles.TMDATemplate.setHierarchyLevel(1,animalKey);
...
handles.TMDATemplate.setHierarchyLevel(2,conditionKey);
...
handles.TMDATemplate.setHierarchyLevel(3,stimKey);
...
handles.TMDATemplate.setHierarchyLevel(4,'LH');
tmdaResults = handles.vdo.getData(handles.TMDATemplate);
xData = tmdaResults(xKey);
fData = tmdaResults(fKey);
if ~isempty(fData)
    set(handles.hideWPCheckbox,'Value',0);
    set(handles.LH_workingPlot,'xdata',xData,'ydata',fData,...
        'visible','on');
...
setappdata(handles.figure1,'LHTMDAResults',tmdaResults);

% Update working plot with current RH data
handles.TMDATemplate.setHierarchyLevel(4,'RH');
tmdaResults = handles.vdo.getData(handles.TMDATemplate);
xData = tmdaResults(xKey);
fData = tmdaResults(fKey);
if ~isempty(fData)
    set(handles.hideWPCheckbox,'Value',0);
    set(handles.RH_workingPlot,'xdata',xData,'ydata',fData,...
        'visible','on');
...
setappdata(handles.figure1,'RHTMDAResults',tmdaResults);
...
switch src
    ...
    case handles.xAxesSelectMenu
        xlim = userSelectAxes('X Axis','ms',{'-1500' '1500'});
        if ~isempty(xlim)
...
handles.TMDATemplate.setHierarchyLevel(4,'LH');
tmdaResults = handles.vdo.getData(handles.TMDATemplate);
lh_xData = tmdaResults(xKey);
lh_fData = tmdaResults(fKey);
lh_data = lh_xData(2:end);
handles.TMDATemplate.setHierarchyLevel(4,'RE');
tmdaResults = handles.vdo.getData(handles.TMDATemplate);
rh_xData = tmdaResults(xKey);
rh_fData = tmdaResults(fKey);
...
varName = sprintf('tmda_%s_%s_%s',animalKey,sessionKey,stimKey);
fprintf('Exporting TMDA data to base workspace as %s\n',varName);
