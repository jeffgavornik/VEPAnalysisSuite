 ...
 *              brackets the event
 * v1.4     J.Gavornik      18 July 2014
 *              Added check to make sure indexing does not extend beyond the 
 *              length of the data arrays to prevent seg faults
...
// [packs,t_vec] = extractPLXEventVEPs(ts_ev,ad_data,t,pack_length)
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    #ifdef DEBUG
    mexPrintf("pre_window = %1.3f, post_window = %1.3f, dt = %f, nEvents = %i, nSamples = %i\n",
        pre_window,post_window,dt,nEvents,nSamples);
    #endif

    if (nSamples==0) mexErrMsgIdAndTxt("extractEventTriggeredTraces:DataError",
        "extractEventTriggeredTraces.cpp:nSamples = 0");
...
static void extractEventVEPs(int nSamples, int nEvents, double *ts_ev,
    ...
    for (int event=0;event<nEvents;event++){
        ...
        while (t[ti]<te-pre_window) {
            ti = ti+1;
            if (ti > nMax){
                mexErrMsgIdAndTxt("extractEventTriggeredTraces:Overflow",
                    "Index extends beyond nSamples");
            }
        }
        ...
        // graphing purposes
        double mv = 0;
        for (int ii=0;ii<10;ii++) mv = mv + ad_data[iData+ii];
        mv = mv/10;
    ...
    #ifdef DEBUG
            mexPrintf("te=%f,evOffset=%i,nSamples*nEvents=%f,iPack=%i,iData=%i\n",
