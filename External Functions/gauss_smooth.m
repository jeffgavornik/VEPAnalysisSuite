function y = gauss_smooth(x,sig,pad_flag,print_flag)
% Function to smooth a function by convolution with a normalized gaussian
% Returns smoothed function with same dimensions as original data
% y = gauss_smooth(x,sig,pad_flag,print_flag)
% x is the data
% sig is stddev for gaussian in same units as data
...
gaussian = normpdf(-sig*padMult:sig*padMult,0,sig); % gaussian kernel
nx = length(x);

if pad_flag ~= 0 % pad data to avoid end effects
    np = sig*padMult; % number of elements in the pad
    if np>nx
        error('gauss_smooth: pad too large for data (sig=%i,np=%i,nx=%i)',sig,np,nx);
    end
...
if print_flag
    ...
    subplot(2,1,2)
    plot(1:length(gaussian),gaussian)
    legend('Kernel')
    xlim(get(sh,'xlim'))
end